...
for test_str in lottery_list:
    ...
    if (length >= 7 and length <= 14):
        ...
        if (output != []):
            print
            "Entry %r is a valid lottery ticket: %r" % (test_str, output)
...
if __name__ == '__main__':
    for test_str in lottery_list:
        ...
        if (length >= 7 and length <= 14):
            output = parse_lottery_ticket(test_str, length)
            if output:
                #print("%r --> %r" % (test_str, ' '.join(output)))
                print(test_str + "->"+' '.join(output))